...
The performance of the robust open-loop MPC can be improved by using closed-loop policy $\mu_i(x_i) : \mathbb{R}^n\to \mathbb{R}^m$ i.e.\ optimize over the sequence
\begin{equation*}
...
\end{equation*}
\begin{itemize}
    \item Optimize over time-varying $K-i$  and $v_i$
...
\begin{itemize}
    ...
    \item Very effective but computationally expensive
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Nominal Control} of the disturbance free system
          \begin{equation*}
              z(k+1) = Az(k) + Bv(k)
          \end{equation*}
    \item \textbf{Disturbance Control} that stabilizes under the disturbance
          \begin{equation*}
              u_i = K(x_i - z_i) + v_i
          \end{equation*}
          where the controller $K$ is computed offline.
\end{enumerate}
...
\noindent\begin{align*}
    e_{i+1} & = x_{i+1} - z_{i+1}                        \\
            ...
            & = (A+BK)e_i + w_i                          \\
            & = A_K e_i + w_i                            \\
            & = \sum_{j=0}^{i-1} A_k^j w_{i-j-1} & e_0=0
\end{align*}
Assuming that $A+BK$ is stable, and the set $\mathcal{W}$ is bounded, we can find the disturbance reachable set (DRS) $\mathcal{F}_i$ that contains all possible states of the error:
\begin{equation*}
...
\textbf{Note} that the set $\mathcal{F}_i$ increases in size with $i$ and converges to $\mathcal{F}_\infty$.
...
The robust constraint-tightening MPC Problem over the nominal state $z$ (tube center) and nominal input $v$ is given by:
\begin{align*}
...
Suppose the terminal ingredients ($I_f, \mathcal{C}_f^{ct}$) are chosen such that
\begin{itemize}
...
\end{equation*}
i.e.\ the problem is recursively feasible ($\mathcal{X}_N$ is the feasible set).

\subsubsection{Tube MPC}
In contrast to constraint-tightening MPC, the tube MPC bounds the maximum error with a fixed size set $\mathcal{E}$, ideally the minimum RPI set $\mathcal{F}_\infty$.

...
\begin{align*}
    ...
    V(g(x)) - V(x)                               \leq -\alpha(x) &  &  & \forall x \in \mathbb{R}^n
\end{align*}
...
\begin{equation*}
    V(x) = x^\top Px
\end{equation*}
as a candidate Lyapunov function with $P\succ0$ (positive definite). It's easy to see that it satisfies
\begin{gather*}
    V(0) = 0 \text{ and } V(x) > 0 \\
...
\begin{equation*}
    {(A+BK)}^\top P{(A+BK)} -P = -Q, \quad Q\succ0
\end{equation*}

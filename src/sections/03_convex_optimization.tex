...
\newpar{}
Hyperplanes are affine and convex, halfspaces are convex. % TODO: issa subsection of convex sets...

\subsubsection{Polyhedra and Polytopes}
Polyhedra and polytopes are always convex. %TODO: else they would not be convex sets, right?
\paragraph{Polyhedra}
...
\begin{center}
    \includegraphics[width = 0.75\linewidth]{03_MinWeyl.png}
\end{center}
...
\end{equation*}
where $x_c$ is the centre of the ellipsoid, and $A \succ 0$ (i.e. $A$ is symmetric and positive definite).   %TODO: I added symmetric
...
But, sublevel sets of $f$ are convex $\forall \alpha \nRightarrow f$ is convex. % isn't the one sided arrow clear enough?
...
\begin{itemize}
    ...
    \item Vector norms on $\mathbb{R}^n$:
          \begin{align*}
              ||x||_p      & ={\left(\sum_{i=1}^n|x|^p\right)}^{1/p},\mathrm{~for~}p\geq1 \\
              ||x||_\infty & =\max_i|x_i|
          \end{align*}
\end{itemize}
\newpar{}
...
\newpar{}
%TODO: why are we only listing some of the possible combinations of magnitudes and signs of two variables (i.e. 4 instead of 2^4)? and why are we listing them at all?
